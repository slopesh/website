  ...
  return (
    <>
      <motion.li
        ...
        initial={{
          transform: `translate${direction === "top" || direction === "bottom" ? "Y" : "X"}(${direction === "top" || direction === "left" ? "-" : ""}30px)`,
          opacity: 0,
        }}
        whileInView={{
          transform: `translate${direction === "top" || direction === "bottom" ? "Y" : "X"}(0px)`,
          opacity: 100,
        }}
        transition={{
          duration: 0.5,
        ...
        >
          ...
          >
            {presence?.activities.map((activity, index) => (
              <div
                key={index}
                className="flex min-[450px]:flex-row flex-col gap-4 items-center px-1 select-none"
              >
                <img
                  alt=""
                  ...
                  <div className="flex flex-col overflow-x-hidden w-full min-[450px]:text-left text-center">
                    ...
                    <div className="flex flex-row gap-2 justify-between mt-1 items-center">
                      ...
                      <div className="w-full rounded-full h-2 bg-secondary overflow-x-hidden">
                        <div
                          style={{
                            width: `${((date.getTime() - new Date(activity.timestamps.start).getTime()) / (new Date(activity.timestamps.end).getTime() - new Date(activity.timestamps.start).getTime())) * 100}%`,
                          }}
